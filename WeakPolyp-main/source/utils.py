...
class TrainData(Dataset):
    def __init__(self, cfg):
        ...
        #         mask  = cfg.train_mask +'/'+folder+'/'+name
...
#     path_dst = path_src.replace('/CVC-ColonDB/', '/CVC-ColonDB-Processed/')
...
#             mask     = cv2.imread(path_src+'/GT/'+folder+'/'+name, cv2.IMREAD_GRAYSCALE)
...
#             cv2.imwrite(path_dst+'/GT/'   +folder+'/'+name, mask)
#             os.makedirs(path_dst+'/Box/'  +folder, exist_ok=True)
#             # cv2.imwrite(path_dst+'/Box/'  +folder+'/'+name.replace('.jpg', '.png'), box)
#             cv2.imwrite(path_dst+'/Box/'  +folder+'/'+name, box)

def preprocess(path_src):
    print('process', path_src)
    # path_dst = path_src.replace('/SUN-SEG/', '/SUN-SEG-Processed/')
    path_dst = path_src.replace('/KiTS19/', '/KiTS19-Processed/')
    # for folder in os.listdir(path_src+'/Frame'):
    #     print(folder)
    for name in os.listdir(path_src+'/Frame'):
            ...
            # cv2.imwrite(path_dst+'/GT/'   +folder+'/'+name.replace('.jpg', '.png'), mask)
            cv2.imwrite(path_dst+'/GT' +'/'+name, mask)
            os.makedirs(path_dst+'/Box/' , exist_ok=True)
            # cv2.imwrite(path_dst+'/Box/'  +folder+'/'+name.replace('.jpg', '.png'), box)
            cv2.imwrite(path_dst+'/Box' +'/'+name, box)